...
import { X, Globe2, Compass, Award, Sparkles, MapPin } from 'lucide-react';
...
export const InfoDrawer: React.FC<InfoDrawerProps> = ({ section, onClose, onBookClick }) => {
  if (!section) return null;

  const contentMap: Record<string, { title: string; subtitle: string; icon: React.ReactNode; body: string[]; points?: string[] }> = {
    'About': {
      title: 'About Tripzy',
      subtitle: 'Inspiring Meaningful Journeys Across the Earth',
      icon: <Award className="w-8 h-8 text-white mb-2" />,
      body: [
    ...
    'Experiences': {
      ...
      icon: <Sparkles className="w-8 h-8 text-white mb-2" />,
      body: [
        'The heart of travel beats in the moments between destinations: learning traditional pasta-making from an Italian nonna, floating in a hot air balloon at sunrise over Cappadocia, or stargazing under Sahara skies.',
    ...
    'Global': {
      ...
      icon: <Globe2 className="w-8 h-8 text-white mb-2" />,
      body: [
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <motion.div 
        ...
        className="relative w-full max-w-lg bg-neutral-950 border border-white/20 rounded-2xl p-6 sm:p-8 text-white shadow-2xl z-10"
      ...
      >
        ...
        <div>{data.icon}</div>
        <h2 className="text-2xl sm:text-3xl font-bold text-white mb-1">{data.title}</h2>
        <p className="text-neutral-400 text-xs sm:text-sm mb-5 uppercase tracking-wider">{data.subtitle}</p>
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex-1 bg-white text-black font-semibold py-2.5 rounded-full text-xs sm:text-sm hover:bg-neutral-200 transition-colors cursor-pointer"
          >
